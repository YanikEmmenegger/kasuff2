// /src/components/quiz/results/ResultMemory.tsx
...
const ResultMemory: React.FC = () => {
    ...
    // Sort answered players by their answer (number of tries), then by answeredAt
    const sortedAnswers = answeredAnswers.sort((a, b) => {
        if (a.answer !== b.answer) {
            return a.answer - b.answer; // Fewer tries (better) first
        } else {
    ...
    return (
        <div className="w-full flex flex-col text-gray-200 p-8">
            <CollapsibleSection title="Show Memory Game Results">
                ...
                <span className="flex items-center w-full justify-between text-lg text-gray-200">
                  ...
                  <span
                      className={`text-lg font-bold ${
                          answer.answer! <= 10 ? 'text-green-300' : 'text-red-300'
                      }`}
                  >
                    {answer.answer}
                  </span>
