// /src/components/quiz/results/ResultSequenceMemory.tsx
...
const ResultSequenceMemory: React.FC = () => {
    ...
    if (!currentAnswers)
        return <div className="text-center text-red-500">No answers available.</div>;
    ...
    // Sort answered players by their answer (number of mistakes), then by answeredAt
    const sortedAnswers = answeredAnswers.sort((a, b) => {
        if (a.answer !== b.answer) {
            return a.answer - b.answer; // Fewer mistakes (better) first
        } else {
    ...
    return (
        <div className="w-full flex flex-col text-gray-200 p-8">
            <CollapsibleSection title="Show Sequence Memory Game Results">
                ...
                <span className="flex items-center w-full justify-between text-lg text-gray-200">
                  ...
                  <span
                      className={`text-lg font-bold ${
                          answer.answer! === 0 ? 'text-green-300' : 'text-red-300'
                      }`}
                  >
                    {answer.answer}
                  </span>
                    ...
                    {playersNotAnswered.length > 0 && <PlayersNotAnswered players={playersNotAnswered}/>}
                </div>
            </CollapsibleSection>
        </div>
    );
...
export default ResultSequenceMemory;
